    ...
    * 更新时间 ：2019/10/7 11:21:26
    ...
    public  class EMMapper
    {
        ...
        private static void Init()
        {
            ...
            moduleBuilder = assemblyBuilder.DefineDynamicModule("KittyModule");
        ...
        private static void Create<V, T>(string name)
        {
            if (typeBuilder == null)
            {
                Init();
         ...
         new Type[] { typeof(V) });
            ...
            il.Emit(OpCodes.Stloc_0);//创建对象后赋值
            //

            var proprtys = typeof(V).GetRuntimeProperties();//获取所有属性
            var vp = typeof(T).GetRuntimeProperties();//获取所有公共属性
            var vf = typeof(T).GetRuntimeFields();//获取所有字段
            foreach (var p in proprtys)
            {
                var r = vp.Where(X => X.Name == p.Name).ToList();//找到相同属性
                if (r.Count > 0)
                {
                    il.Emit(OpCodes.Ldloc_0);
                    il.Emit(OpCodes.Ldarg_0);//准备赋值
                ...
                {
                    var fields = vf.Where(X => X.Name.ToLower() == p.Name.ToLower()).ToList();//找到相同字段名称
                    if (fields.Count > 0)
                    {
                        il.Emit(OpCodes.Ldloc_0);
